...
const char *typename[] = {"UNDEF_TYPE", "INT_TYPE", "REAL_TYPE", "CHAR_TYPE"};
...
int typecheck(int type1, int type2)
{
    ...
    if (type1 == REAL_TYPE && type2 == REAL_TYPE) return REAL_TYPE;
    if (type1 == CHAR_TYPE && type2 == CHAR_TYPE) return CHAR_TYPE;

    /* guard against out-of-range */
    int t1 = (type1 >= 0 && type1 <= 3) ? type1 : 0;
    int t2 = (type2 >= 0 && type2 <= 3) ? type2 : 0;
